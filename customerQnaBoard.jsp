...
<%
	...
	String search = request.getParameter("qnaSearch");
	List<BoardDTO> allCustomerBoards = new ArrayList<BoardDTO>();
	
	BoardDAO bdao = new BoardDAO();
	allCustomerBoards = bdao.getBoard("qna", search);
...
%>
<input type="hidden" name="boardDtlType" id="boardDtlType" value="qna">
<div style="width:800px; height:46px; margin-top:16px" align="center">
	<div align="center">
		<div class="qnaMenu" style="cursor:pointer; width: 250px;border: 1px solid #cccccc;float:left;padding-top: 14px;padding-bottom: 25px;border-radius: 15px;position: absolute;margin-left: 15px;background-color: #f7f7f7;font-size: 15px;color: #797979">질문게시판</div>
		...
		<table width="188" class="table_basic" cellpadding="0" cellspacing="0" style="margin-top:20px; background:url('images/main/searchbar3.png'); background-size:100%; height:38px; background-repeat:no-repeat" align="right">
			<tr height="38">
				<td width="158" align="right" style="vertical-align:top">
					<input type="text" id="qnaSearch" name="qnaSearch" placeholder="제목 검색" style="font-family:'나눔고딕'; font-size:13px; padding-top:2px; padding-left:14px; width:144px; height:27px; background-color:transparent; outline:0; border:0; color:#808080" value="${search}">
				</td>
				<td width="30" style="vertical-align:top"><a id="qnaBtn"><img src="images/main/searchbtn.png" width="16" style="margin-left:4px; margin-top:5px; cursor:pointer"></a></td>
			</tr>
		</table>
	</div>
			...
			<tr height="30">
				<c:forEach var="page" begin="1" end="${pgList.pageCount}" step="1">
					<c:if test="${pgList.pageNum==page}">
						<td class="qnaPaging" style="background-color:#f2f2f2; font-size:11px; vertical-align:middle" width="31" align="center">${page}</td>
					</c:if>
					<c:if test="${pgList.pageNum!=page}">
						<td class="qnaPaging" style="font-size:11px; vertical-align:middle; cursor:pointer" width="31" align="center">${page}</td>
					</c:if>
				</c:forEach>
			</tr>
		</table>
