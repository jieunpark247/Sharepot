...
<%
	...
	String search = request.getParameter("recommendSearch");
	List<BoardDTO> allCustomerBoards = new ArrayList<BoardDTO>();
	
	BoardDAO bdao = new BoardDAO();
	allCustomerBoards = bdao.getBoard("recommend", search);
...
%>
<input type="hidden" name="boardDtlType" id="boardDtlType" value="recommend">
<div style="width:800px; height:46px; margin-top:16px" align="center">
	<div align="center">
		...
		<div class="reportMenu" style="cursor:pointer; width: 250px;border: 1px solid #cccccc;float:left;padding-top: 14px;padding-bottom: 25px;border-radius: 15px;position: absolute;margin-left: 274.3px;font-size: 15px;color: #797979" align="center">고장신고</div>
		<div class="recommendMenu" style="cursor:pointer; width: 250px;border: 1px solid #cccccc;float:left;padding-top: 14px;padding-bottom: 25px;border-radius: 15px;position: absolute;margin-left: 534px;font-size: 15px;color: #797979;background-color: #f7f7f7" align="center">물품제안</div>
	</div>
...
<div>
		...
		<table width="188" class="table_basic" cellpadding="0" cellspacing="0" style="margin-top:20px; background:url('images/main/searchbar3.png'); background-size:100%; height:38px; background-repeat:no-repeat" align="right">
			<tr height="38">
				<td width="158" align="right" style="vertical-align:top">
					<input type="text" id="recommendSearch" name="recommendSearch" placeholder="제목 검색" style="font-family:'나눔고딕'; font-size:13px; padding-top:2px; padding-left:14px; width:144px; height:27px; background-color:transparent; outline:0; border:0; color:#808080" value="${search}">
				</td>
				<td width="30" style="vertical-align:top"><a id="recommendBtn"><img src="images/main/searchbtn.png" width="16" style="margin-left:4px; margin-top:5px; cursor:pointer"></a></td>
			</tr>
		</table>
	</div>
	<div style="height:${40*(rows2+1) + 58}">
		<table class="content-table" width="800" border="1" frame="void" rules="rows" cellpadding="0" cellspacing="0" style="margin-top:0px">
			<tr height="40" style="background-color:#f6f6f6">
				...
				<td id="table-td" class="table-line" width="70">추천수</td>
							...
							<table style="padding:0px">
								<tr>
									<td style="padding-right:0px">
										<a class="boardDtlBtn" style="cursor:pointer"><input type="hidden" name="customerQnaHidden" id="customerQnaHidden" value="${allCustomerBoard.getNo()}">${allCustomerBoard.getTitle()}<c:if test="${allCustomerBoard.getReplyCnt() != 0}"><font style="font-weight:bold"> (${allCustomerBoard.getReplyCnt()})</font></c:if></a>
									</td>
								</tr>
							</table>
						</td>
	...
	<div align="right" style="margin-top:10px; margin-bottom:-40px">
		<input class="writeBoard" type="button" value="글쓰기" style="width:80px;height:30px;background-color: #ababab;border:0;border-radius: 5px;font-size: 13px;color:#ffffff;font-family:'나눔고딕';cursor:pointer;margin-right: 7px;margin-top: 1px;">
		<input type="hidden" id="hiddenType" name="hiddenType" value="recommend"><input type="hidden" id="sessionHidden" name="sessionHidden" value="${sessionScope.idKey}" />
	</div>
			...
			<tr height="30">
				<c:forEach var="page" begin="1" end="${pgList.pageCount}" step="1">
					<c:if test="${pgList.pageNum==page}">
						<td class="recommendPaging" style="background-color:#f2f2f2; font-size:11px; vertical-align:middle" width="31" align="center">${page}</td>
					</c:if>
					<c:if test="${pgList.pageNum!=page}">
						<td class="recommendPaging" style="font-size:11px; vertical-align:middle; cursor:pointer" width="31" align="center">${page}</td>
					</c:if>
				</c:forEach>
			</tr>
		</table>
