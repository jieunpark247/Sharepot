...
<%
	...
	String search = request.getParameter("reportSearch");
	List<BoardDTO> allCustomerBoards = new ArrayList<BoardDTO>();
	
	BoardDAO bdao = new BoardDAO();
	allCustomerBoards = bdao.getBoard("report", search);
...
%>
<input type="hidden" name="boardDtlType" id="boardDtlType" value="report">
...
<div>
		...
		<table width="188" class="table_basic" cellpadding="0" cellspacing="0" style="margin-top:20px; background:url('images/main/searchbar3.png'); background-size:100%; height:38px; background-repeat:no-repeat" align="right">
			<tr height="38">
				<td width="158" align="right" style="vertical-align:top">
					<input type="text" id="reportSearch" name="reportSearch" placeholder="제목 검색" style="font-family:'나눔고딕'; font-size:13px; padding-top:2px; padding-left:14px; width:144px; height:27px; background-color:transparent; outline:0; border:0; color:#808080" value="${search}">
				</td>
				<td width="30" style="vertical-align:top"><a id="reportBtn"><img src="images/main/searchbtn.png" width="16" style="margin-left:4px; margin-top:5px; cursor:pointer"></a></td>
			</tr>
		</table>
	</div>
			...
			<c:set value="${pgList.startRow - 1}" var="no" />
				...
				<c:forEach var="allCustomerBoard" items="${allCustomerBoards}" begin="${pgList.startRow - 1}" end="${pgList.endRow - 1}">
					...
					<tr height="40">
						...
						<td id="table-td" class="table-line">${allCustomerBoard.getOption()}</td>
	...
	<div align="right" style="margin-top:10px; margin-bottom:-40px">
		...
		<input type="hidden" id="hiddenType" name="hiddenType" value="report"><input type="hidden" id="sessionHidden" name="sessionHidden" value="${sessionScope.idKey}" />
	</div>
			...
			<tr height="30">
				<c:forEach var="page" begin="1" end="${pgList.pageCount}" step="1">
					<c:if test="${pgList.pageNum==page}">
						<td class="reportPaging" style="background-color:#f2f2f2; font-size:11px; vertical-align:middle" width="31" align="center">${page}</td>
					</c:if>
					<c:if test="${pgList.pageNum!=page}">
						<td class="reportPaging" style="font-size:11px; vertical-align:middle; cursor:pointer" width="31" align="center">${page}</td>
					</c:if>
				</c:forEach>
			</tr>
		</table>
